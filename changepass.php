...
<?php 
  ...
  {  
     ...
     $sta=$_POST["status"];
     $uid=$_SESSION["id"];
     // bewaar hier de post input in de post tabel
  }
...
<?php
 ...
 if(isset($updpass))
 {
 	$em=$_SESSION["email"];
  //$pass=md5($op);
  $passwordHash = password_hash($np, PASSWORD_DEFAULT);
 
	$que=mysqli_query($con,"select * from registration where email='$em'");
    ...
    if(strlen($cp) < 6){
      $err="<font color='red'>Password must be atleast 6 characters!</font>";
		...
		{
      $que=mysqli_query($con,"update registration set password='$passwordHash' where email='$em'");
      $err="<font color='green'>You have successfully changed your password!</font>";
    }
	 else
	 {
	  $err="<font color='red'>New Password and Confirm Password does not match!</font>";
	 }
  }
 }
 ?>
